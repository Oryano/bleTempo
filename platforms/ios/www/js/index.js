...
var app = {
	...
	bindEvents: function() {
			...
			this.CallSendData = function(_test){
				console.log('callsenddata with "that"')
				that.sendData(_test);
			}
		...
		//testDevice.addEventListener('touchstart', this.sendData, false);

		pushToDevice.addEventListener('touchstart', function(){
			console.log("pushToDeviceButton");
			that.CallSendData(false);
			console.log("false");
			},false);

		testDevice.addEventListener('touchstart', function(){
			console.log("testDeviceButton");
				that.CallSendData(true);
				console.log("true");
				},false);
		...
		connect: function(e) {
			...
			onConnect = function() {
				...
				testDevice.dataset.deviceId = deviceId; //added for test button
		...
		sendData: function(test) { //a function that calc random RGB and sends to device.-
			console.log("SEDNDATA function called");
	    var deviceId = event.target.dataset.deviceId; // first find out device ID
			...
			while(completeTime.length < 8){ //make sure 8 digits (add zeros)
		  ...
		  //////---------------------- 234 digits
			var fullBits = Math.floor(digits_345 / 255); //devide each part to by the corrisponding amoutof bits
			var restBit = digits_678 % 255; 		//mudule the spliced number in 255
			...
			if (ble) ble.write(deviceId, scratchServiceUUID, scratchCharacteristicUUID, data.buffer, app.onSuccess(data), app.onError);
		},
		...
		onSuccess: function(data){
			if(data[3] == 0){
	    alert("Timestamp in ms: " + data[0] + data[1] + data[2] +"\n"+ "Pattern: "+"pattern"+ "\n"+"Frequency: "+"freq"+"\n"+"Time manipulation: "+ "bool");
			}else{
			alert("Running test pattern");
			}
		}
	};
